...
if [[ -f "$(dirname "${BASH_SOURCE[0]}")/lib/v6-event.sh" ]]; then
  ...
  v6_pipeline_stage_started "stage=evening-sprint-collate cron_at=$(date -u +%Y-%m-%dT%H:%M:%SZ)"
  trap 'v6_pipeline_stage_completed "stage=evening-sprint-collate exit=$?"' EXIT
fi
